...
namespace AVD.Common.Logging
{
    ...
    public sealed class Logger
    {
        ...
        private Logger(String type)
        {
            ...
            var dummyElasticSearchInstance = new log4net.ElasticSearch.ElasticSearchAppender();
        ...
        public void Error(string className, string methodName, Exception exception, string message = "")
        {
            ...
            if (message == string.Empty)
            {
                string logMessage = string.Format(ExceptionOnlyMessageFormat, className, methodName, exception.Message + " " + exception.StackTrace);
                Log4NetLogger.Error(logMessage, exception);
            }
            else
            {
                string logMessage = string.Format(ExceptionLogMessageFormat, className, methodName, message, exception.Message + " " + exception.StackTrace);
                Log4NetLogger.Error(logMessage, exception);
            }
        }
